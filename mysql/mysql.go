 ...
 * @LastEditTime: 2019-11-24 14:06:27
...
func QueryArticleList(limit int, lastSid int) ([]model.ArticleStruct, error) {
	...
	var err error
	if lastSid != 0 {
		err = db.Model(&model.ArticleStruct{}).Select("sid, title, pub_time, summary, thumb, comment_count, url").Where("sid < ?", lastSid).Order("sid desc").Limit(limit).Find(&articleList).Error
	} else {
		err = db.Model(&model.ArticleStruct{}).Select("sid, title, pub_time, summary, thumb, comment_count, url").Order("sid desc").Limit(limit).Find(&articleList).Error
	}
	return articleList, err
}
